...
class Mesh:
    ...
    def uv_to_xyz(self, points_in_2d_in: np.ndarray, planary_uv: np.ndarray, num_attempts=100):
        """
        ...
        # Guard: if ALL points were deleted, downstream code will crash
        # on empty arrays (IndexError, mean-of-empty-slice, etc.).
        # Raise explicitly so the caller can handle it cleanly.
        if points_out_3d.shape[0] == 0:
            raise ValueError(
                f"uv_to_xyz: all {points_in_2d_in.shape[1]} input points "
                f"failed to map to mesh triangles (num_attempts={num_attempts})."
            )
