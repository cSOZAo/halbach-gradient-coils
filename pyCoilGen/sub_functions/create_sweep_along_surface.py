...
def create_sweep_along_surface(coil_parts: List[CoilPart], input_args) -> List[CoilPart]:
    ...
    if not input_args.skip_sweep:
        conductor_resistivity = input_args.specific_conductivity_conductor
        for part_ind in range(len(coil_parts)):
            ...
            """
            # Guard: wire path must have at least 2 points for direction computation
            if wire_path.v.shape[1] < 2:
                raise ValueError(
                    f"create_sweep_along_surface: wire_path has only "
                    f"{wire_path.v.shape[1]} points — need at least 2."
                )

            # Calculate the length of the coil
            wire_path.v_length = np.sum(np.linalg.norm(wire_path.v[:, 1:] - wire_path.v[:, :-1], axis=0))
            # Guard: cross-section area must be positive to compute resistance
            ...
            # First pass: compute normals where face lookup succeeds
            failed_indices = []
            for point_ind in range(wire_path.v.shape[1]):
                ...
                if node_ind_normals_target == -1:
                    failed_indices.append(point_ind)
                else:
                    surface_normal_along_wire_path_v[:, point_ind] = coil_mesh.fn[node_ind_normals_target]

            # Second pass: fill failed normals from nearest valid neighbour
            for point_ind in failed_indices:
                if point_ind == 0:
                    # Borrow from the first future point that has a valid normal
                    for future in range(1, wire_path.v.shape[1]):
                        if future not in failed_indices:
                            surface_normal_along_wire_path_v[:, point_ind] = surface_normal_along_wire_path_v[:, future]
                            break
                else:
