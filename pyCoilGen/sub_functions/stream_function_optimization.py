...
def reduce_matrices_for_boundary_nodes(full_mat, coil_mesh, set_zero_flag):
    ...
    if np.any(dim_to_reduce):
        ...
        for dim_to_reduce_ind in np.where(dim_to_reduce)[0]:
            ...
            # Trim the matrix instead of using np.delete (avoids full-copy
            # memory fragmentation on large matrices)
            keep_size = num_boundaries + len(is_not_boundary_node)
            Index6 = [slice(None)] * np.ndim(full_mat)
            Index6[dim_to_reduce_ind] = slice(0, keep_size)
            reduced_mat = reduced_mat[tuple(Index6)].copy()
