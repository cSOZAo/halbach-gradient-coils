...
def _coil_only_wire_paths(folder: str) -> list[str]:
    ...
    from output_utils import _is_derived_wire_stl

    paths: list[str] = []
    for wire in glob.glob(os.path.join(folder, 'Gradient_*_wire_0_?.stl')):
        if not _is_derived_wire_stl(wire):
            paths.append(wire)
    return paths
...
def _seed_wire_from_previous_run(run_dir: str) -> bool:
    ...
    for wire in _coil_only_wire_paths(best_dir):
...
def _ensure_wire_exists(run_dir: str) -> None:
    ...
    print("Expected pyCoilGen export "
          f'Gradient_G{cfg.GRADIENT_AXIS}_tk{int(cfg.TIKHONOV_FACTOR)}_lvl{cfg.NUM_LEVELS}'
          f'_wire_0_<internal_axis>.stl in {run_dir}')
