...
class Config:
    ...
    def cylinder_design_radius(self) -> float:
        ...
        Base: ``cylinder.radius - semi_a``.  Offset for gradient centering:
        ``(1 - margin_pct) * cable_height + gap / 2``.
        """
        base = self.cylinder.radius - self.conductor_semi_a
        margin = self.shell.auto_margin_pct
        gap = self.winding.layer_gap_mm
        return base + (1.0 - margin) * self.cable_height + 0.5 * gap
    ...
    def shell_wall_thickness(self) -> float:
        """Radial wall thickness: 2h + gap − 2×radial_peel."""
    ...
    def shell_outer_radius(self) -> float:
        """Outer shell radius (= GUI ``cylinder.radius``)."""
        return self.cylinder.radius

    @property
    def shell_inner_radius(self) -> float:
        """Inner bore radius: R − (2h + gap − 2×radial_peel)."""
